...
public class TestGui {
    ...
    public void init() {
        ...
        for (Class<?> clazz : sortedClasses) {
            ...
            GuiComponent classComponent = new GuiComponent(screen, (screen.width - 70) / 5split screen in 5, (int) (screen.height * 0.0416) 45);
            ...
            classComponent.shapes(guiComponent -> {
                ...
                new Rectangle(guiComponent, classComponent.width, classComponent.height, new BColor(0.52f, 0.52f, 0.52f, 1))
                        .radius(12)
                        .shade(2);
                new Text(guiComponent, 1, name, new BColor(0, 0, 0, 1))
                        .offset(10, 5);
            });
            classComponent.listen2Click(guiComponent -> {
                classComponent.childrenVisible = !classComponent.childrenVisible;
            });
            ...
            for (Field field : fields) {
                ...
                switch (fieldType) {
                    case BUTTON:
                        createButton(clazz, field, fieldComponent, classComponent);
                        break;
                    case SLIDER:
                        // Handle slider field
                        break;
                    case TEXT_INPUT:
                        ...
                        break;
                    case COMING_SOON:
                        createComingSoon(clazz, field, fieldComponent, classComponent);
                        break;
                }
            }
        }
    ...
    private void createComingSoon(Class<?> clazz, Field field, GuiComponent fieldComponent, GuiComponent classComponent) {
        fieldComponent.shapes(guiComponent -> {
            ...
            BColor color = new BColor(0.1f, 0.1f, 0.1f, 1);

            new Rectangle(guiComponent, color)
                    .radius(20)
                    .shade(3);

            new Text(guiComponent, 1, name, new BColor(0, 0, 0, 1))
                    .offset(10, fieldComponent.height/6);
        });
        fieldComponent.setVisible(classComponent.childrenVisible);
        components.add(fieldComponent);
    }

    private void createButton(Class<?> clazz, Field field, GuiComponent fieldComponent, GuiComponent classComponent) {
    ...
    private void setupBackground(){
        screen.shapes(guiComponent -> {
            new Rectangle(guiComponent, new BColor(0.12f, 0.12f, 0.12f, 0.32f));  // Makes background darker
        });

        GuiComponent text = new GuiComponent(screen, (screen.width - 70) / 5split screen in 5, (int) (screen.height * 0.0416) 45);
        text.offset(10, screen.height - text.height - 10);
        text.shapes(guiComponent -> {
            new RRectangle(guiComponent,  new BColor(0.52f, 0.52f, 0.52f, 1))
                    .radius(12)
                    .shade(2);

            new Text(guiComponent, 1, "GROMOD PRE-ALPHA", new BColor(0, 0, 0, 1))
                    .offset(10, text.height/8);;
        });
